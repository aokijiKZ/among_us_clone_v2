...
func _ready() -> void:
	...
	if OS.has_environment("USERNAME"):
		username_text_edit.text = OS.get_environment("USERNAME")
	else:
		var desktop_path = OS.get_system_dir(0).replace("\\", "/").split("/")
		username_text_edit.text = desktop_path[desktop_path.size() - 2]
...
#		Global.player_name = username_text_edit.text
#		Profile.set_profile_data('player_name', username_text_edit.text)
		get_tree().change_scene("res://waiting_room/waiting_room.tscn")
