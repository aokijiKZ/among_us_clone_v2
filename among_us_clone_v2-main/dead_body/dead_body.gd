...
#		get_node('/root/multiplayer').rpc('switch_to_meeting',name.rsplit('_',true,1)[1])

func _on_report_area_2d_body_entered(body):
...
func _on_report_area_2d_body_exited(body):
	if body.is_in_group('Player'):
		if get_tree().has_network_peer():
			if body.is_network_master():
...
func _on_report_area_2d_input_event(viewport, event, shape_idx):
	if player_network_master == null:
		return
